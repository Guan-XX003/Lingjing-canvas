...
export const wanjuanI18n = {
      "zh-TW": {
        ...
        "1.4.0：新增邮箱账号与本机安全凭据存储、企业局域网网关和成员配额管理；完善全局批量配置切换，保留用户其他 API 配置，并支持卸载时清理本地用户数据。": "1.4.0：新增信箱帳號與本機安全憑證儲存、企業區域網路閘道和成員配額管理；完善全域批次配置切換，保留使用者其他 API 配置，並支援解除安裝時清理本機使用者資料。",
      ...
      "en-US": {
        ...
        "1.4.0：新增邮箱账号与本机安全凭据存储、企业局域网网关和成员配额管理；完善全局批量配置切换，保留用户其他 API 配置，并支持卸载时清理本地用户数据。": "1.4.0: Added email accounts, secure local credential storage, enterprise LAN gateways, and member quotas; improved global preset switching while preserving unrelated API configs, and added local user-data removal during uninstall.",
