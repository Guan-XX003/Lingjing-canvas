...
import { buildConfigButlerToolContext, buildLocalConfigButlerErrorDiagnosis, formatConfigButlerToolContext, inferButlerCategoryFromModelName, mergeButlerProtocolRepair, normalizeButlerBaseUrl, normalizeConfigButlerDiagnosis, normalizeModelCategory, validateAndRepairConfigButlerResult } from "../lib/config-butler";
...
- 修复一个字段时必须保留当前协议中与本次错误无关的有效字段、类型、参数适配器和响应映射，禁止用不完整模板覆盖完整协议。
...
${String(promptText || ``).slice(0, 5e4)}`;
		                let diagnosisRaw = await Promise.race([
		                  ...
		                  validateAndRepairConfigButlerResult({
		                    ...
		                    category: modelCategory,
		                    protocol: {
		                      ...jsonBlock.suggestedProtocol,
		                      config: mergeButlerProtocolRepair(protocolConfig || task.requestProfile || {}, jsonBlock.suggestedProtocol.config, task.errorMsg || ``),
		                    },
		                  }, {
		                    modelName: task.modelName,
		                    category: modelCategory,
