...
const tools = [
  { name: "wanjuan_status", description: "检查StarCanvas桌面应用是否已启动并可接受自动化任务。", inputSchema: { type: "object", properties: {}, additionalProperties: false } },
  { name: "wanjuan_list_models", description: "列出StarCanvas当前配置的图片、视频和文本模型，不返回 API Key。", inputSchema: { type: "object", properties: {}, additionalProperties: false } },
  { name: "wanjuan_list_tasks", description: "列出StarCanvas任务清单中的生成任务。", inputSchema: { type: "object", properties: {}, additionalProperties: false } },
  { name: "wanjuan_generate_image", description: "使用StarCanvas创建文生图或参考图生图任务。返回 nodeId，可用于查询或等待任务。", inputSchema: { type: "object", required: ["prompt"], properties: { prompt: { type: "string", description: "图片提示词" }, model: { type: "string", description: "模型名称；留空使用应用首选模型" }, size: { type: "string", default: "1024x1024" }, referenceImage: { type: "string", description: "可选参考图片绝对路径或 URL" } }, additionalProperties: false } },
  { name: "wanjuan_generate_video", description: "使用StarCanvas创建文生视频或图生视频任务。返回 nodeId，可用于查询或等待任务。", inputSchema: { type: "object", required: ["prompt"], properties: { prompt: { type: "string", description: "视频提示词" }, model: { type: "string", description: "模型名称；留空使用应用首选模型" }, resolution: { type: "string", default: "1280x720" }, duration: { type: "number", description: "视频时长（秒）" }, aspectRatio: { type: "string", description: "例如 16:9" }, referenceImage: { type: "string", description: "图生视频输入图片绝对路径或 URL" } }, additionalProperties: false } },
  ...
  { name: "wanjuan_cancel_task", description: "取消正在运行的StarCanvas生成任务。", inputSchema: { type: "object", required: ["id"], properties: { id: { type: "string" } }, additionalProperties: false } },
];
...
lines.on("line", async (line) => {
  ...
  try {
    if (method === "initialize") return result(id, { protocolVersion: String(params.protocolVersion || "2025-06-18"), capabilities: { tools: { listChanged: false } }, serverInfo: { name: "wanjuan-lingjing", version: "1.0.0" }, instructions: "通过本机StarCanvas应用执行图片和视频生成。生成前可先调用 wanjuan_list_models。" });
